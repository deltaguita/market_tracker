...
class MercariScraper:
    ...
    def _call_search_api(self, page: Page, keyword: str) -> List[Dict]:
        ...
        try:
            ...
            # 從頁面中獲取 searchSessionId 和 laplaceDeviceUuid
            session_data = page.evaluate("""
                () => {
                    return {
                        searchSessionId: window.__NEXT_DATA__?.props?.pageProps?.searchSessionId || 
                                        Math.random().toString(36).substring(2, 15),
                        laplaceDeviceUuid: localStorage.getItem('laplaceDeviceUuid') || 
                                          'bba571c3-8a4d-451b-bbd7-a77da06ae65f'
                    };
                }
            """)

            search_session_id = session_data.get("searchSessionId", "")
            device_uuid = session_data.get(
                "laplaceDeviceUuid", "bba571c3-8a4d-451b-bbd7-a77da06ae65f"
            )
