...
def run_source(
    source: str, force: bool = False, headless: bool = True, dry_run: bool = False
) -> bool:
    """
    執行單一來源的爬取
    ...
    if url_index_filter is not None:
        try:
            ...
            if 0 <= url_index < len(config.tracking_urls):
                ...
                print(
                    f"Processing only URL at index {url_index}: {urls_to_process[0].name}"
                )
            else:
                print(
                    f"URL_INDEX {url_index} out of range (0-{len(config.tracking_urls) - 1}), processing all URLs"
                )
        except ValueError:
    ...
    for url_config in urls_to_process:
        ...
        try:
            ...
            result = storage.compare_products(
                current_products, source, config.price_tracking_mode
            )
            ...
            if max_threshold is not None:
                ...
                filtered_new = [
                    ...
                    if p.get(price_field, 0) > 0
                    and p.get(price_field, 0) <= max_threshold
                ]
                filtered_dropped = [
                    item
                ...
                print(
                    f"New products (total: {len(new_products)}, within budget: {len(filtered_new)})"
                )
                print(
                    f"Price dropped (total: {len(price_dropped)}, within budget: {len(filtered_dropped)})"
                )
    ...
    if (all_new_products or all_price_dropped) and notifier:
        ...
        for product, max_threshold in all_new_products:
            ...
            if notifier.notify_new_product(
                product, source=source, is_within_budget=is_within_budget
            ):
                success_count += 1
...
範例:
  ...
  %(prog)s --list                 # 列出所有可用來源
    ...
    parser.add_argument(
        "--force", "-f", action="store_true", help="強制執行，忽略排程檢查"
    )
    parser.add_argument(
        "--headed", action="store_true", help="以有頭模式運行瀏覽器（用於除錯）"
    )
    parser.add_argument(
        "--dry-run", "-n", action="store_true", help="測試模式，不發送通知"
    )
    parser.add_argument(
        "--status", "-s", action="store_true", help="顯示來源的排程狀態"
    )
    parser.add_argument("--list", "-l", action="store_true", help="列出所有可用來源")
    ...
    success = run_source(
        args.source, force=args.force, headless=headless, dry_run=args.dry_run
    )
