...
class DatabaseMigrator:
    ...
    def migrate(self) -> None:
        ...
        for version in range(current_version + 1, target_version + 1):
            if version in self._migrations:
                ...
                logger.warning(
                    f"No migration function registered for version {version}, skipping"
                )
    ...
    def _execute_migration(
        self, version: int, migration_func: Callable[[sqlite3.Connection], None]
    ) -> None:
        """
        安全執行單一遷移
    ...
    def _create_v2_schema(self, conn: sqlite3.Connection) -> None:
        ...
        注意：此方法假設 products 表不存在或已經是 V2 結構。
        如果表存在且是 V1，應先透過 _migrate_v1_to_v2() 進行遷移。
        ...
        if products_exists:
            ...
            if "source" not in columns:
                ...
                raise MigrationError(
                    "Products table exists but is not V2 schema. "
                    "This should be handled by _migrate_v1_to_v2(). "
        ...
        # 建立 schedule_state 表（用於記錄各來源的上次執行時間）
        cursor.execute("""
        ...
        """)
        logger.debug("Created schedule_state table")
